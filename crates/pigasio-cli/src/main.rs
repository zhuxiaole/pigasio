...
# wasapi 能通过 IAudioClient3 压低共享模式的设备周期(见下一项),但能不能
# 受益**取决于驱动** —— 相当多的消费级声卡只支持默认的 10 ms。
# 改动后要重启宿主才生效(驱动跑在宿主进程里)。
# 环境变量 PIGASIO_BACKEND 的优先级比这一项高,方便临时覆盖排查。
backend = "cpal"

# 请求的设备周期(帧)。省略 = 保持设备默认的周期。
# 只有 wasapi 后端认这一项;设了才会去降。**降周期有风险** —— 实测某些虚拟
# 声卡在小周期下产出速率偏离标称,会导致环形缓冲持续溢出。改完先用
# `pigasio check` 跑一遍,确认「溢出」还是 0 再实际使用。
# 周期越短延迟越低,CPU 占用和爆音风险越高;降下来之后记得把上面的
# watermark_ms 一起调小,否则总延迟省不下来。
# period_frames = 128
