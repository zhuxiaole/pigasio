...
pub struct EngineConfig {
    ...
    /// 请求的设备周期(帧)。**不写就保持设备默认的周期。**
    ///
    /// 只有 WASAPI 后端认这一项 —— 它靠 `IAudioClient3` 把共享模式的 period
    /// 降下来;cpal 后端忽略它(共享模式的周期由 audio engine 定)。
    ///
    /// 设了才会去降,而且**降周期是有风险的**:实测 Virtual Audio Cable 在它的
    /// 最小 period(48 帧)下产出速率比标称高约 3%,远超漂移补偿的能力(默认
    /// 500 ppm),环形缓冲会一路积压到溢出。所以这个开关默认是关的,想压延迟
    /// 的人自己打开、自己验证。
    ///
    /// 周期越短延迟越低,CPU 占用和爆音风险越高。降下来之后**记得把
    /// `watermark_ms` 也一起调小** —— 水位至少要盖过两块设备回调,设备块小了
    /// 水位就该跟着降,否则延迟省不下来。
