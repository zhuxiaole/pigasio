//! 设备枚举与配置匹配。
//!
//! 配置里的设备用“名字片段”描述,因为 Windows 上设备的持久标识符
//! (`IMMDevice` 的 endpoint id)会随驱动更新而变,写死在配置里很容易失效。
//! 这里负责把描述解析成实际可以打开的**设备句柄**。
//!
//! 真正跟音频 API 打交道的是 [`crate::backend`];本模块只做**匹配** ——
//! 把后端枚举出来的设备列表,按配置的描述挑出一个。

use crate::backend;
use crate::config::{DeviceRef, StreamConfig};
use crate::error::{Error, Result, StreamKind};

/// 一次枚举得到的设备信息。定义在 [`crate::backend`] 里 —— 后端负责构造它,
/// 这里只是转出来方便调用方使用。
pub use crate::backend::DeviceInfo;

/// 枚举指定方向上的所有设备。
///
/// 单个端点查询失败不会中断整个流程:Windows 上偶尔会有端点处于异常状态,
/// 后端会跳过它并记一条日志,而不是让驱动初始化彻底失败。
pub fn enumerate(kind: StreamKind) -> Result<Vec<DeviceInfo>> {
    backend::current().enumerate(kind)
}
...
pub fn default_device(kind: StreamKind) -> Result<DeviceInfo> {
    backend::current().default_device(kind)
}
