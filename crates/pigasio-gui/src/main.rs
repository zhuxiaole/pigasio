...
impl App {
    fn draw_engine_settings(&mut self, ui: &mut egui::Ui) {
        ...
        // 说明文字与上面两栏之间留出可读的间距。
        //
        // 这里原来是 2.0,几乎贴着 —— 而右栏的最后一行是「实测最大设备块」那行
        // 提示(右栏比左栏多一行,而 `columns` 的高度由较高的一栏决定),于是两段
        // 小字直接挤在一起,分不清哪句属于哪段。
        ui.add_space(8.0);
        ui.label(
            egui::RichText::new(
    ...
    fn engine_settings_right(&mut self, ui: &mut egui::Ui) {
        egui::Grid::new("engine_settings_right")
            ...
            .show(ui, |ui| {
                ...
                .on_hover_text(
                    "请求的设备周期(帧)。0 = 保持设备默认的周期。\n\n\
                     只有「直写 WASAPI」后端认这一项 —— 它把共享模式的 period 降到\
                     这个量级;其他后端由系统决定,通常是 10 ms。\n\n\
                     **降周期有风险**:实测某些虚拟声卡在小周期下产出速率偏离标称,\
                     会导致环形缓冲持续溢出。设定之后先用「试运行」确认「溢出」还是 0。\n\n\
