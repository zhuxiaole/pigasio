...
设备块降下来之后,`watermark_ms` 就成了延迟的主要剩余部分 —— 而它只需要盖过
约两块设备回调。控制面板的试运行面板现在会拿**实测设备块**和当前水位比,分
三种情况给结论:

| 情况 | 显示 |
|---|---|
| 水位 < 一块设备块 | 黄色警告:必然欠载,给出建议下限 |
| 水位 > 下限的两倍 | 蓝色提示:给出可降到的目标值和能省下的毫秒数 |
| 其余 | 灰色的中性倍数 |

推荐下限取 `2.3 × 设备块`(两块再加 15% 余量),出自 README 里那段实测结论。
判定逻辑抽成了 `watermark_verdict()` / `watermark_floor_ms()` 两个纯函数,
有单元测试覆盖三档和边界。

**没有做自动下调。** 那需要在真实机器上积累欠载数据才能定出安全的收敛策略,
现阶段给建议、由用户自己确认更稳妥。
...
- 阶段 3 的代码路径在开发机上只走通了"退回默认"那一支(因为驱动不支持),
  ...
  同理,水位联动的蓝色提示在这台机器上也触发不了(设备块恒 10 ms,默认水位
  30 ms 落在"合理"区间)。
